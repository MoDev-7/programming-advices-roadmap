...
bool isDateWithinPeriod(stPeriod p, stDate d) {
    return !IsDate1BeforeDate2(d, p.start) && !IsDate1AfterDate2(d, p.end);
}
int main()
{
    stPeriod p1 = readFullPeriod();
    stDate d = readFullDate();
    cout << (isDateWithinPeriod(p1, d) ? "yeah" : "nah") << endl;
}